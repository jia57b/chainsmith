#!/bin/bash
# init-multinode.sh — Initialize 4-validator Cronos localnet
#
# Architecture: Cosmos SDK + CometBFT + integrated EVM (single process)
# Each validator = 1 container (consensus + EVM in the same cronosd process)
#
# Usage:
...
IMAGE="${CRONOS_IMAGE:-chainsmith/cronosd:local}"
CRONOS_TAG="${CRONOS_TAG:-v1.7.0}"
...
# Amounts (18 decimals)
VALIDATOR_BALANCE="10000000000000000000000${DENOM}"
VALIDATOR_STAKE="100000000000000000000${DENOM}"
FOUNDER_BALANCE="100000000000000000000000${DENOM}"
...
run_cronos() {
  local vol="$1"
  shift
  docker run --rm --user root --entrypoint cronosd \
    -v "${vol}:${CRONOS_HOME}" "$IMAGE" "$@" --home "${CRONOS_HOME}"
}
...
run_cronos_quiet() {
  ...
  shift
  docker run --rm --user root --entrypoint cronosd \
    -v "${vol}:${CRONOS_HOME}" "$IMAGE" "$@" --home "${CRONOS_HOME}" >/dev/null 2>&1
}
...
run_genesis_cmd() {
  ...
  shift
  docker run --rm --user root --entrypoint cronosd \
    -v "${vol}:${CRONOS_HOME}" "$IMAGE" genesis "$@" --home "${CRONOS_HOME}" >/dev/null 2>&1 || \
  docker run --rm --user root --entrypoint cronosd \
    -v "${vol}:${CRONOS_HOME}" "$IMAGE" "$@" --home "${CRONOS_HOME}" >/dev/null 2>&1
}

echo "============================================"
echo "  Cronos Multi-Validator Localnet Init"
echo "  Architecture: CometBFT + Integrated EVM"
echo "  Validators: ${NUM_VALIDATORS}"
echo "  Image: ${IMAGE}"
echo "  Source Tag: ${CRONOS_TAG}"
...
if ! docker image inspect "${IMAGE}" >/dev/null 2>&1; then
  echo "   Image ${IMAGE} not found locally, building from source tag ${CRONOS_TAG}..."
  docker build -t "${IMAGE}" \
    --build-arg CRONOS_TAG="${CRONOS_TAG}" \
    "${SCRIPT_DIR}/docker/node"
fi
echo "   ✅ Image ready"
echo ""
...
for i in $(seq 1 $NUM_VALIDATORS); do
  run_cronos_quiet "cronos_validator${i}_home" init validator${i} --chain-id "${CHAIN_ID}"
...
for i in $(seq 1 $NUM_VALIDATORS); do
  ...
  echo -e "password123\npassword123" | docker run -i --rm --user root --entrypoint cronosd \
    -v cronos_validator${i}_home:${CRONOS_HOME} \
    "$IMAGE" keys unsafe-import-eth-key validator${i} ${PRIVKEY} \
      --keyring-backend test --home ${CRONOS_HOME} >/dev/null 2>&1 || true

  ADDR=$(docker run --rm --user root --entrypoint cronosd \
    -v cronos_validator${i}_home:${CRONOS_HOME} \
    "$IMAGE" keys show validator${i} \
      --keyring-backend test --home ${CRONOS_HOME} -a 2>/dev/null | tr -d '\n\r')
...
echo -e "password123\npassword123" | docker run -i --rm --user root --entrypoint cronosd \
  -v cronos_validator1_home:${CRONOS_HOME} \
  "$IMAGE" keys unsafe-import-eth-key founder "${FOUNDER_ETH_PRIVKEY}" \
    --keyring-backend test --home ${CRONOS_HOME} >/dev/null 2>&1 || true

FOUNDER_ADDR=$(docker run --rm --user root --entrypoint cronosd \
  -v cronos_validator1_home:${CRONOS_HOME} \
...
run_genesis_cmd "cronos_validator1_home" add-genesis-account "${FOUNDER_ADDR}" "${FOUNDER_BALANCE}" --keyring-backend test
...
for i in $(seq 1 $NUM_VALIDATORS); do
  ...
  if [ $i -eq 1 ]; then
    run_genesis_cmd "cronos_validator1_home" add-genesis-account validator1 "${VALIDATOR_BALANCE}" --keyring-backend test
  else
    run_genesis_cmd "cronos_validator1_home" add-genesis-account "${ADDR}" "${VALIDATOR_BALANCE}" --keyring-backend test
  fi
...
docker run --rm --user root --entrypoint sh \
  -v cronos_validator1_home:${CRONOS_HOME} \
  "$IMAGE" -c '
    GENESIS='"${CRONOS_HOME}"'/config/genesis.json
    jq '"'"'
      .app_state.staking.params.bond_denom = "'"${DENOM}"'" |
      (if .app_state.gov.deposit_params? then .app_state.gov.deposit_params.min_deposit[0].denom = "'"${DENOM}"'" else . end) |
      (if .app_state.gov.params? then .app_state.gov.params.min_deposit[0].denom = "'"${DENOM}"'" else . end) |
      (if .app_state.evm?.params? then .app_state.evm.params.evm_denom = "'"${DENOM}"'" else . end) |
      (if .app_state.feemarket?.params? then .app_state.feemarket.params.base_fee = "'"${BASE_FEE}"'" else . end) |
      (if .consensus_params?.block? then .consensus_params.block.max_gas = "30000000" else . end) |
      (if .consensus?.params?.block? then .consensus.params.block.max_gas = "30000000" else . end)
    '"'"' "$GENESIS" > "${GENESIS}.tmp" && mv "${GENESIS}.tmp" "$GENESIS"
  ' >/dev/null 2>&1

echo "   ✅ Genesis patched (bond_denom/evm_denom/base_fee/max_gas)"
echo ""
...
for i in $(seq 2 $NUM_VALIDATORS); do
  docker run --rm --user root \
    -v cronos_validator1_home:/src:ro \
    -v cronos_validator${i}_home:/dst \
...
for i in $(seq 1 $NUM_VALIDATORS); do
  run_genesis_cmd "cronos_validator${i}_home" gentx validator${i} "${VALIDATOR_STAKE}" \
...
for i in $(seq 2 $NUM_VALIDATORS); do
  docker run --rm --user root \
    -v cronos_validator${i}_home:/src:ro \
    -v cronos_validator1_home:/dst \
    alpine sh -c "cp /src/config/gentx/* /dst/config/gentx/" 2>/dev/null
done

run_genesis_cmd "cronos_validator1_home" collect-gentxs
run_genesis_cmd "cronos_validator1_home" validate || run_genesis_cmd "cronos_validator1_home" validate-genesis || true

for i in $(seq 1 $NUM_VALIDATORS); do
  docker run --rm --user root --entrypoint sh \
    -v cronos_validator${i}_home:${CRONOS_HOME} \
    "$IMAGE" -c '
      CFG='"${CRONOS_HOME}"'/config/config.toml
      APP='"${CRONOS_HOME}"'/config/app.toml
      ...
      sed -i "s|minimum-gas-prices = \"0stake\"|minimum-gas-prices = \"0${DENOM}\"|" "$APP"
    ' >/dev/null 2>&1
...
PEERS=""
for i in $(seq 1 $NUM_VALIDATORS); do
  NODE_ID=$(docker run --rm --user root --entrypoint cronosd \
    -v cronos_validator${i}_home:${CRONOS_HOME} \
    "$IMAGE" comet show-node-id --home ${CRONOS_HOME} 2>/dev/null || \
  docker run --rm --user root --entrypoint cronosd \
    -v cronos_validator${i}_home:${CRONOS_HOME} \
    "$IMAGE" tendermint show-node-id --home ${CRONOS_HOME} 2>/dev/null)

  NODE_ID=$(echo "$NODE_ID" | tr -d '\n\r')
  PEER="${NODE_ID}@cronos-validator${i}:26656"
