#!/bin/bash
# prepare-cold-validator.sh — Start a temporary AvalancheGo node, keep it running,
# and persist a manifest for cold-join onboarding tests.
#
# Usage:
#   ./prepare-cold-validator.sh
#
# Output:
#   chains/avalanche-cli-local/runtime/cold-validator/cold-validator-manifest.json
...
RUNTIME_DIR="${SCRIPT_DIR}/runtime/cold-validator"
MANIFEST_PATH="${RUNTIME_DIR}/cold-validator-manifest.json"
PROCESS_PATH="${RUNTIME_DIR}/process.json"
MAIN_LOG_PATH="${RUNTIME_DIR}/logs/main.log"
SANITIZED_FLAGS_PATH="${RUNTIME_DIR}/cold-flags.json"
...
if not primary_nodes:
    raise SystemExit("No primary nodes discovered; cannot prepare cold-join validator candidate")
...
manifest = {
    ...
    "source": "prepare-cold-validator.sh",
    "pid": process.pid,
    "nodeId": node_id,
...
echo ""
echo "✅ Cold-join validator manifest written:"
echo "   ${MANIFEST_PATH}"
echo ""
