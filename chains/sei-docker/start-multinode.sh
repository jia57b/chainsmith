#!/bin/bash
# start-multinode.sh — Start Sei 4-validator localnet
#
# Prerequisites:
...
echo "🚀 Starting Sei multi-validator localnet..."
# Start validator1 first so P2P listeners are up before other nodes dial persistent peers.
docker compose --env-file "${SCRIPT_DIR}/.env.multinode" -f "$COMPOSE_FILE" up -d sei-validator1
echo "   Waiting for first validator P2P to listen..."
sleep 8
...
while [ $WAITED -lt $MAX_WAIT ]; do
    # Sei RPC returns a flat JSON body (no .result); other chains nest under .result
    HEIGHT=$(curl -s http://localhost:26657/status 2>/dev/null | \
      jq -r '.sync_info.latest_block_height // .result.sync_info.latest_block_height // empty' 2>/dev/null | tr -d '"')
    HEIGHT=${HEIGHT:-0}
...
if [ $WAITED -ge $MAX_WAIT ]; then
    ...
    echo "   1. Check logs:   docker compose --env-file .env.multinode -f docker-compose.yml logs sei-validator1"
...
echo ""
echo "✅ Sei multi-validator localnet started!"
echo ""
echo "📊 Network Status:"
echo "   Block Height:    ${HEIGHT}"
